...
# 平均休日数を計算
avg_rest_days = len(days) * (1 - len(shifts) / len(employees))

# 目的関数：休日の偏りとペナルティの最小化
prob += (
    pulp.lpSum(rest_deviation[e] for e in employees) +
    10 * pulp.lpSum(night_shift_penalty[e] for e in employees) +
    5 * pulp.lpSum(early_late_penalty[e] for e in employees) +
    50 * pulp.lpSum(shift_deviation[e][s] for e in employees for s in ["早番", "遅番"])
...
# 休日数の偏差を計算
for e in employees:
    prob += rest_deviation[e] >= rest_days[e] - avg_rest_days
    prob += rest_deviation[e] >= avg_rest_days - rest_days[e]
...
# 3. 夜勤と夜勤明けはセット
for e in employees:
    for d in range(1, 30):  # 29日目まで
        if d + 1 in days:
            prob += assignment[e]["夜勤"][d] == assignment[e]["夜勤明け"][d+1]

# 4. 最大4連勤
for e in employees:
    for d in range(1, 27):  # 25日目まで
...
# 9. 山宮、湯沢、荒牧、伊坪は日勤のみ
day_shift_only_employees = ['山宮', '湯沢', '荒牧', '伊坪']
...
# 10. 早番と遅番は連続2日まで
for e in employees:
    for d in range(1, 29):  # 28日目まで（3日連続を防ぐため）
...
if pulp.LpStatus[prob.status] == "Optimal":
    ...
    print("\n休日数の偏差:")
    for e in employees:
        print(f"{e}: {rest_deviation[e].value():.2f}")
else:
