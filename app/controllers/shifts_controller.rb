  ...
  def update_schedule
    ...
    ActiveRecord::Base.transaction do
      ...
      params[:memos].each do |date, content|
        memo = Memo.find_by(date: date, user_id: current_user.id)
        if content.blank?
          memo&.destroy
        else
